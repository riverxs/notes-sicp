...
function accumulate(filter, combiner, null_value, term, a, next, b) {
	...
	if(filter(term(a))){
		return combiner(term(a), accumulate(filter, combiner, null_value, term, next(a), next, b))
	}else{
...
function sum2(term, a, next, b) {
	...
	return combiner(term(a), accumulate(isPrime, combiner, 0, term, next(a), next, b))
}
...
function product2(term, a, next, b){
    ...
    return combiner(term(a), accumulate(filterTerm, combiner, 1, term, next(a), next, b))
}
